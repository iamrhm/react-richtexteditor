...
const TagSugWithHOC = withConsumer(({
  ...
  const onQueryChange = async (
    searchText: string, offsetKey: string, triggerKey: string,
  ): Promise<void> => {
    ...
    const { suggestions, showHint } = await fetchSuggestions(
      searchText,
      state.activeTriggerKey,
    );
  ...
  const onEditorStateChange = React.useCallback(() => {
    if (store.editorState) {
      const query = getSuggestionQuery(
        store.editorState,
        store.tagState.searchKeys,
        store.possibleTriggerKeys,
      );
  ...
  React.useEffect(() => {
    if (editorContainerRef?.current) {
      editorContainerRef.current
        .addEventListener('scroll', hideSuggestions);
    }
    return (): void => {
      if (editorContainerRef?.current) {
        editorContainerRef.current
          .removeEventListener('scroll', hideSuggestions);
      }
    };
  }, [editorContainerRef?.current]);
...
export default React.forwardRef<HTMLDivElement, IProps>((props, ref) => {
  ...
  return (
    <TagSugWithHOC editorContainerRef={innerRef} {...props} />
  )
});
