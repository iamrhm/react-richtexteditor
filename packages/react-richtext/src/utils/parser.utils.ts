...
export function parseEditorData(
  ...
  [...Array.from(previewList)]
    .forEach(([, value]) => {
      const linkKey = value.info?.viewText ?
        `${value.info.viewText}${linkCount}` : `${value.info.url}${linkCount}`;
  ...
  const body = currentContent.blocks.map((block) => {
    ...
    if (block.entityRanges.length) {
      block.entityRanges.forEach((entityRange) => {
        ...
        const taggedTextKey = Array.from(block.text).slice(offset, offset + length).join('').trim();
        ...
        if (richAssetData && richAssetData[taggedTextKey]) {
          ...
          `${objOfAssets[`${taggedTextKey}${assetCount}`].id}${assetCount}`;
    ...
    if (matchArr) {
      matchArr.forEach((match) => {
        ...
        if (linkText !== null && linkText !== '' && objOfLinks[`${linkText}${count}`]) {
          blockText = blockText.replace(
            linkText,
            `[[[#entityType${objOfLinks[`${linkText}${count}`].id}]]]`,
          );
          count += 1;
        }
      });
  ...
  Object.keys(objOfAssets)
    .forEach((key) => { entityMap[objOfAssets[key].id] = objOfAssets[key]; });
